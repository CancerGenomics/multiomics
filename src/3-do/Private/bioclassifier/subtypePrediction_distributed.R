# File copied from bioclassifier.
# MDB: Modifies the generated heat map to show the name if the samples. 
# 
###############################################################################
...
subtypePrediction_distributed <- function(expressionMatrix, mirnaMatrix, ncolForExpressionId){
...
myHeatmap(out$testData,cbind(subtypeColors,conf.colors),file=paste(clustername,".cdt",sep=""),rowNames=rownames(out$testData), colNames=colnames(out$testData))
dev.off()
