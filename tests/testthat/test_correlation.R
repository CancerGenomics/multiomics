...
test_that("WCGNA correlation gives same results as previous implementation", {
  ...
  calculated.with.wcgna <- CalculateCorrelationsMirnaMrnaUsingWCGNA(mrna.dif.expr,mirna.dif.expr, working.path, 
  ...
  expect_equal(dim(calculated.legacy),dim(calculated.with.wcgna))
